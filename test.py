...
def main(args):
    ''' Create Dir for Save '''
    _, _, result_dir = create_dirtest(args)
    ...
    ''' MODEL LOADING '''
    print('\nModel Initial ...')
    if args.model_name.lower() == 'bicubic':
        MODEL_PATH = 'model.' + args.task + '.' + args.model_name
        MODEL = importlib.import_module(MODEL_PATH)
        net = MODEL.BicubicUpsample(scale_factor=args.scale_factor)
        # Assuming BicubicUpsample is already defined and imported
    else:
        MODEL_PATH = 'model.' + args.task + '.' + args.model_name
        MODEL = importlib.import_module(MODEL_PATH)
